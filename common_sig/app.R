...
server <- function(input, output) {
  ...
  observe({
    ...
    #shinyDirChoose(input, 'folder', roots=c(wd='.'), filetypes=c("","csv"))
  ...
  analysis <- observeEvent(input$run, {
    ...
    # apply the function (f) to each row in the table and get the experiments names
    # and than take only unique names
    lst <- lapply(apply(snps, 1, f), unique)

    # for each row add the experiment name(s)
    for (i in 1:length(lst)){
      experiments <- as.character(lst[i])
      experiments <- sub("c(", "", experiments, fixed = TRUE)
      experiments <- sub(")", "", experiments, fixed = TRUE)
      snps$Exp[i] <- experiments
    }
    ...
    # display the table
    output$snp_table <- renderDT(datatable(snps, options = list(
                                                             columnDefs = list(
