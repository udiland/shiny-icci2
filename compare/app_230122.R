...
server <- function(input, output) {
  ...
  gwas_res <- observeEvent(input$run, {
    ...
    # create the graph
    p <- ggplot(dat, aes(x=BPcum, y=-log10(FDR_Adjusted_P.values), text=text, customdata=link, shape=model))+ 
        geom_jitter(aes(color=as.factor(Chromosome)), size=3, height = 0.5, width = 0.9) +
      ...
      scale_y_continuous(expand = c(0, 0), limits = c(min(-log10(all$FDR_Adjusted_P.values)) - 0.5, max(-log10(all$FDR_Adjusted_P.values)) + 0.5)) +
