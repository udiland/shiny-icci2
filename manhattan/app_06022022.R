...
library(mapview)
library(ggthemes)
source("/srv/shiny-server/manhattan/all2geno.R")
library(gridExtra)
...
geo_data <- read.csv("/srv/shiny-server/manhattan/geo_data2.csv")
...
pheno <- req(reactive({
  ...
  ext <- tools::file_ext(file$datapath)
  pheno_tbl <- read.table(file$datapath, sep='\t', head=T, na.strings= "NA")
  
  #give new column name
  colnames(pheno_tbl)[1] <- "accession"
  colnames(pheno_tbl)[2] <- "phenotype"
  # remove na
  pheno_tbl <- na.omit(pheno_tbl)
  pheno_tbl
}))
...
#    })
  ...
  geno_pheno_geo <- req(reactive(merge(values$res[input$tbl2_rows_selected ,], geo_data, by="accession", all.x=T)))
