  ...
  tags$body(
    ...
    fluidRow(
      column(3,
             ...
             numericInput(inputId = "end", label = "Choose end position", 1e6),
...
server <- function(input, output) {
  ...
  analysis <- observeEvent(input$runManhattan, {
    ...
    p <- ggplot(don, aes(x=BPcum, y=-log10(P.value), text=text, customdata=link, shape=FDR_Adjusted_P.values < 0.05)) +
      ...
      geom_hline(yintercept = -log10(min(don$P.value[don$FDR_Adjusted_P.values > 0.05])), linetype="dashed",
                 color = "red", size=1) +
...
function(el) {
  ...
  observeEvent(input$runPhenotype, {
      ...
      if(input$chr_data == "pruned"){
        if(input$chr == "Un"){
          snps <- read.table("/home/udiland/gwas/snp/data/geno/all_samples/LD_pruned_100/long_chr8.hmp.txt", sep = '\t', comment.char = "", head=T)
          
          #snps <- read.table("/Users/udila/Downloads/long_chr8.hmp.txt", sep = '\t', comment.char = "", head=T)
          
        } else if(input$chr == "All"){
          snps <- read.table("/home/udiland/gwas/snp/data/geno/all_samples/LD_pruned_100/long_all_pruned.hmp.txt", sep= "\t", comment.char = "", head=T)
          
        } else(
          snps <- read.table(paste("/home/udiland/gwas/snp/data/geno/all_samples/LD_pruned_100/long_chr",input$chr,".hmp.txt", sep = ""), sep = '\t', comment.char = "", head=T)
        )
      }
      if(input$chr_data == "filtered"){
        if(input$chr == "Un"){
          snps <- read.table("/home/udiland/gwas/snp/data/geno/all_samples/long_chr8.hmp.txt", sep = '\t', comment.char = "", head=T)
          
        } else if(input$chr == "All"){
          snps <- read.table("/home/udiland/gwas/snp/data/geno/all_samples/long_all_pruned.hmp.txt", sep= "\t", comment.char = "", head=T)
          
        } else(
          snps <- read.table(paste("/home/udiland/gwas/snp/data/geno/all_samples/long_chr",input$chr,".hmp.txt", sep = ""), sep = '\t', comment.char = "", head=T)
        )
      }
    ...
    file <- input$gwas
    ext <- tools::file_ext(file$datapath)
    gwas <- read.csv(file$datapath)
    
    # change "UN" to "Un"
    gwas$Chromosome[gwas$Chromosome == "UN"] <- "Un"
    
    
    #show the table of selected range range
    selected_pos <- snps[snps$pos >= input$start & snps$pos <= input$end ,]
  ...
  observe({
    if (length(snp_rows()) == 1 ){
        ...
        ext <- tools::file_ext(file$datapath)
        pheno <- read.table(file$datapath, sep='\t', head=T)
      ...
      geno <- values$snps[values$snps$pos == position ,][c(12 : length(values$snps))]
    ...
    if (length(snp_rows()) > 1){
      ...
      # get genotype data from snp table using the snp position (subset from column number 12..)
      geno <- values$snps[values$snps$pos %in% positions ,][c(4, 12 : length(values$snps))]
      ...
      # add snp name
      all <- merge(req(genotype), values$res, by ="accession")
  ...
  geno_pheno_geo <- reactive(merge(values$res[input$tbl2_rows_selected ,], req(geo_data), by="accession", all.x=T))
  ...
  output$leafletMap <- renderLeaflet(map() %>% addCircleMarkers(data = geno_pheno_geo(),
