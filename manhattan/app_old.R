...
function(el) {
    ...
    if(input$chr_data == "pruned"){
    	if(input$chr == "Un"){
     		 ...
     		 #hapmap <- read.table("/Users/udila/Downloads/long_chr8.hmp.txt", sep = '\t', comment.char = "", head=T)
      			...
      			hapmap <- read.table("/storage/udiland/gwas/snp/data/geno/all_samples/LD_pruned_100/long_all_pruned.hmp.txt", sep= "\t", comment.char = "", head=T)
   ...
   if(input$chr_data == "filtered"{
        if(input$chr == "Un"){
                hapmap <- read.table("/storage/udiland/gwas/snp/data/geno/all_samples/long_chr8.hmp.txt", sep = '\t', comment.char = "", head=T)
                        ...
                        hapmap <- read.table("/storage/udiland/gwas/snp/data/geno/all_samples/long_all_pruned.hmp.txt", sep= "\t", comment.char = "", head=T)

                } else(
                        read.table(paste("/storage/udiland/gwas/snp/data/geno/all_samples/long_chr",input$chr,".hmp.txt", sep = ""), sep = '\t', comment.char = "", he$
                        )
  ...
  geno_pheno_geo <- reactive(req(merge(values$res[input$tbl2_rows_selected ,], geo_data, by="accession", all.x=T)))
